...
class InterStationsScreen(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['SetupActions', 'ColorActions'], {'yellow': self.saveParameters,
         'ok': self.selstation,
         'cancel': self.close}, -2)
...
def Plugins(**kwargs):
    return PluginDescriptor(name='Greek Net Radio', where=[PluginDescriptor.WHERE_EXTENSIONSMENU, PluginDescriptor.WHERE_PLUGINMENU], description=_('Listen to your greek music'), icon='greekradio.png', fnc=main)
